# Qwen Forge Documentation

**v0.1.2-beta**

---

## Documents

| Section | English | Russian |
|---------|---------|---------|
| Installation | [installation.en.md](./installation.en.md) | [installation.md](./installation.md) |
| CLI | [cli.en.md](./cli.en.md) | [cli.md](./cli.md) |
| Architecture | [architecture.en.md](./architecture.en.md) | [architecture.md](./architecture.md) |
| Development | [development.en.md](./development.en.md) | [development.md](./development.md) |
| Troubleshooting | [troubleshooting.en.md](./troubleshooting.en.md) | [troubleshooting.md](./troubleshooting.md) |
| FAQ | [faq.en.md](./faq.en.md) | [faq.md](./faq.md) |
| Contributing | [contributing.en.md](./contributing.en.md) | [contributing.md](./contributing.md) |
| Changelog | [changelog.en.md](./changelog.en.md) | [changelog.md](./changelog.md) |

## Quick links

- [README.md](../README.md) — main project README
- `config.json` — runtime configuration, created on first launch
- [install.sh](../install.sh) — installation script
